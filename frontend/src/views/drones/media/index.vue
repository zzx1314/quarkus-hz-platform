...
import { useDronesMedia } from "./hook";
...
import Dowload2Line from "~icons/ri/download-2-line";
import { message } from "@/utils/message";
import { SUCCESS } from "@/api/base";
import { dronesMediaUpdate } from "@/api/dronesMedia";
...
const beforeUpload = (uploadFile: UploadFile, uploadFiles: UploadFiles) => {
  ...
  if (
    uploadFile.name.includes(".mp3") ||
    uploadFile.name.includes(".mp4") ||
    uploadFile.name.includes(".mov")
  ) {
    uploadFileTemp.value = uploadFile;
  } else {
    uploadRef.value!.clearFiles();
    message("请选择正确的安装包格式：.mp3 或 .mp4 或 .mov", {
      type: "error"
    });
  }
};
...
const submitUploadUpdate = () => {
  if (uploadFileTemp.value !== null) {
    ...
    dronesMediaUpdate(addForm.value).then(res => {
      if (res.code === SUCCESS) {
...
<template>
  <div class="main">
    ...
    <PureTableBar title="媒体库列表" :columns="columns" @refresh="onSearch">
      <template #buttons>
        <el-button
          type="primary"
        ...
        >
          <template #operation="{ row }">
            ...
            <el-button
              ...
              type="primary"
              :size="size"
              :icon="useRenderIcon(Dowload2Line)"
    ...
    >
      <el-form
        ...
        label-width="150px"
      >
        <el-form-item label="文件描述" prop="remarks" style="width: 100%">
          <el-input
